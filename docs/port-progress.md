...
|---|---|---|---|
| 2026-03-06 | **daTitle Draw SIGSEGV investigation + defensive improvements**: Traced frame_0200=0% regression after f41e8a4 tev_reg_dirty fix. Root cause (from 6fac29b1): clearEfb z=0 depth corruption blocks daTitle draws via LEQUAL. Applied four improvements: (1) `apply_rasc_color` alpha fix — both ambient and grey-fallback paths now force `const_clr[3]=255` (opaque) so the fallback colour is actually visible on screen. (2) BCK/BRK/BPK/BTK animation `calc()` stubs — `J3DMtxCalcAnimationAdaptorDefault::calc()`, `J3DTevColorAnm::calc()`, `J3DMatColorAnm::calc()`, `J3DTexMtxAnm::calc()`, `J3DTexNoAnm::calc()` return early on `PLATFORM_PC` to prevent SIGSEGV from reading big-endian keyframe data. (3) `PLATFORM_PC=1` added as explicit CMake compile-definition for all PC TUs — ensures header-only templates (J3DJoint.h, J3DMaterialAnm.h) see the flag even in TUs that don't transitively include global.h. `global.h` updated with `#ifndef` guards to avoid redefinition. (4) daTitle_draw diagnostic improved — now also logs `proc` (mProcID state-machine value) and `dc` (gx_frame_draw_calls snapshot). **Remaining SIGSEGV**: daTitle Draw() still crashes at `mDoExt_modelUpdateDL(mpModel)` even without entry() calls, because `J3DJoint::mCurrentMtxCalc` (a static shared across all actors) is left pointing at another actor's big-endian BCK animation. Added `setMtxCalc(NULL)` on joint 0 but crash persists. Root cause NOT fully resolved this session. | 17 (stable) | **Next session MUST**: (1) Fix daTitle SIGSEGV — crash is in J3DModel::update() because `J3DJoint::mCurrentMtxCalc` (static) points at a stale big-endian anim from a previous actor. Solution: reset the static to NULL in `mDoExt_bckAnm::remove()` call pattern — or add a `J3DJointTree::resetCurrentMtxCalc()` PC-only call before `mDoExt_modelUpdateDL`. (2) Verify depth fix (6fac29b1) makes frame_0200 > 0% after SIGSEGV is resolved. (3) Add frame_0200 regression gate once confirmed. |
|---|---|---|---|
