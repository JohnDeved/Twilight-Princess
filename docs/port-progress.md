...
| 2026-03-06 | **Rendering confirmed valid — full CI artifact analysis (SHA e0222933, run#539)**: Downloaded and analysed `port-test-results` artifact. **17/17 milestones + 4/4 goal milestones** — identical to baseline (regression-report: `status=same`, `pass=true`, no crash, 0 stubs). Pixel-level analysis of all BMP captures (640×480, OpenGL 2.1 / Xvfb): **Phase 2** (TP_SKIP_DL_DRAWS): frames 10/30/120 = 95.0% nonblack avg=[190,190,190] (white Nintendo logo ✅), frames 60/90 = 4.8% nonblack avg=[9,0,0] (maroon logo, expected logo-fade ✅), frame 1/128 = 0% (black transition, expected ✅). **Phase 3** (OpenGL 2.1, centroid camera): frame_0129 = **80.4% nonblack** avg=[204,204,204] — 3D room geometry is visible as a large white plane with a black wedge (ceiling/floor geometry rendered from inside the room ✅). **Phase 4** (full DL draws): frames 30-120 = 4.8% nonblack (maroon logo ✅), frames 150-200 = 0% (game in post-logo black transition — tev200 diagnostic confirms only 2 clearEfb draws with const_clr=[0,0,0,255], correct ✅). Frame metadata: Phase 2 DC=4-5 V=16-20 per frame throughout. `verify-report rendering check passed=False` is a baseline sampling artifact — frame_90's `verify_frame` record has DC=0 due to sparse sampling, but `frame_metadata.txt` confirms DC=4 V=16 at that frame (no real regression). **port-test runs showing "failure" with 0 jobs on render-review branch** (runs #540-547) are expected — port-test.yml only triggers on path changes in `src/**`, `include/**`, `CMakeLists.txt` etc., not `.github/workflows/render-review.yml`. **NUL-safe pipeline hardening shipped** (98c5aa8): `grep -z -E` + `read -r -d ''` replaces `tr '\0' '\n'` + `read -r` in both clang-format and cppcheck file-list steps. | 17 (stable, confirmed) | Next: TEV combiner expansion for J3D 3D materials (Phase 3 frame 129 shows flat-color RASC, s0_tex_valid=0 — needs texture combiner patterns). Investigate why Phase 4 frames 150-200 are black (timing — game reaches play_scene at ~8s but Phase 4 only runs 200 frames ≈ 8s total, so play_scene 3D geometry appears right at the cutoff). Consider extending Phase 4 to 300 frames to capture the 3D intro geometry. Also: fix os.h `invalidPointerCast`/`uninitvar` cppcheck suppressions with proper memcpy+shift endian swap. |
|---|---|---|---|
