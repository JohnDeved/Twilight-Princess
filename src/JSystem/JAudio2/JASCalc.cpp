...
void JASCalc::bcopyfast(const void* src, void* dest, u32 size) {
    JUT_ASSERT(226, (((uintptr_t)src) & 0x03) == 0);
    JUT_ASSERT(227, (((uintptr_t)dest) & 0x03) == 0);
...
void JASCalc::bcopy(const void* src, void* dest, u32 size) {
    ...
    u8 endbitsSrc = ((uintptr_t)bsrc & 0x03);
    u8 enbitsDst = ((uintptr_t)bdest & 0x03);
...
void JASCalc::bzerofast(void* dest, u32 size) {
    JUT_ASSERT(336, (((uintptr_t)dest) & 0x03) == 0);
...
void JASCalc::bzero(void* dest, u32 size) {
    u32* udest;
    u8* bdest = (u8*)dest;
    if ((size & 0x1f) == 0 && (((uintptr_t)dest) & 0x1f) == 0) {
        DCZeroRange(dest, size);
        return;
    }

    u8 alignedbitsDst = ((uintptr_t)bdest) & 0x3;
