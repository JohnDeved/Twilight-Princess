...
| **Current Step** | Step 5+ — J3D texture binding + material color injection complete |
| **Last Updated** | 2026-03-01 |
| **Blocking Issue** | Runtime verification needed — texture + material color pipeline connected |
...
|---|---|---|---|
| 2026-03-01 | **J3D texture binding + material color pipeline**: (1) Texture pointer table — J3DGDSetTexImgPtr/PtrRaw register 64-bit pointers in lookup table (up to 4096 entries), store compact ID in BP command's 24-bit data, resolve during DL replay. (2) BP TexImage register parsing — TexImage0 (0x88-0x8F: width/height/format), TexImage3 (0x94-0x9B: image pointer via table), TexMode0 (0x80-0x83: wrap modes). (3) XF register parsing — material ambient colors (0x100A), diffuse colors (0x100C), channel count (0x1009), texgen config (0x1040-0x1047), numTexGens (0x103F). (4) Material color injection — MODULATE/BLEND/DECAL with no vertex color now use material color from channel control (matching GCN hardware). (5) Fixed inject_color texcoord handling — was losing UVs for textured draws. (6) Load texcoord + CLR1 arrays per-frame from J3DVertexData on PC (CP 0xA0+ array commands have truncated physical addresses). (7) TLUT guard for J3DGDLoadTlut. (8) J3DGDSetTexLookupMode writes BP commands AND sets state. | 14 (no change) | Runtime verification with game data to see title screen models |
