...
| **Active CI** | Rendering CONFIRMED VALID (CI run#539, SHA e0222933). 17/17 milestones, 4/4 goal milestones. No crash, 0 stubs. See session log 2026-03-06 rendering verification entry. |
...
| **Known rendering gap** | Phase 2 (softpipe, TP_SKIP_DL_DRAWS=1) title/logo frames 10/30/120 = 95% nonblack ✅. Frame 60/90 = ~4% nonblack (logo fade animation, expected, documented in render-baseline.json). Phase 4 (all draws) frames 30-120 = 4% nonblack (logo visible). Frame 200: expected to improve after depth fix (daTitle model should now be visible via apply_rasc_color fallback → gray ~200,200,200). |
...
| **DL parser SDK fixes** | ✅ DONE: Fixed 10 bugs by verifying DL parser against dolsdk2004 source: alpha compare bit layout, konst selector BP address (0xF6-0xFD not 0x18-0x1F), konst color vs TEV color register distinction, channel control defaults (mat_src=VTX, num_chans=0), XF channel control registers (0x100E-0x1011), XF light memory (0x0600-0x067F), GEN_MODE cull mode with FRONT↔BACK swap, TEV order bump alpha mapping, CP MatIdxA/B, GXSetFogColor. | Done | P0 done |
...
| 2026-03-07 | **Render-path regression narrowed and partially corrected (logo back, intro frame no longer just clear color)**: Local reference runs against `origin/copilot/sub-pr-14` showed the working behavior for this stacked PR: Phase 4 frame_0030 uses `fs_gx_blend` and renders a 4% nonblack red Nintendo logo; Phase 3 frame_0129 is 79% nonblack. Current branch had regressed in two ways: (1) view 1 depth+color clearing was wiping view 0 every frame, so logo/UI frames could never survive; (2) `detect_tev_preset()` was force-upgrading any non-trivial alpha compare to `fs_gx_tev`, which misrouted the logo’s 2-stage `[TEXC,0,0,0]` + `[C0,C1,CPREV,0]` pattern away from `fs_gx_blend`. Fixes landed in this session: (a) restored view 1 to **depth-only** clear so centroid-camera room draws composite over view 0 instead of overwriting it, (b) stopped forcing alpha-compare draws through `fs_gx_tev`, letting the title/logo path use the same `fs_gx_blend` preset as the working base branch, (c) taught `classify_tev_config()` that the single-stage `[TEXC,0,0,0]` form is also REPLACE. **Local verification:** current branch now matches base-branch logo behavior at frame_0030/0060/0090/0120 (`pct_nonblack=4`, avg `[9,0,0]`, red logo colors present) and Phase 3 frame_0129 improved from the flat clear-color failure to **56% nonblack** (`frame_0129.bmp` top colors: white/black), which confirms real scene content is reaching the framebuffer again. | 17 (stable locally) | **Next session**: (1) close the remaining Phase 3 gap versus base branch (56% local vs 79% on `sub-pr-14`) by comparing centroid-camera state / TEV submit counts around frame 128-129, (2) inspect why Phase 4 frame_0128+ is still black after the logo sequence despite restored logo-path blending, (3) if the new local behavior is stable in CI, raise/update the notes around title/logo visibility to reflect the recovered frame_0030 output. |
| 2026-03-07 | **Headless fade handling refined for logo visibility + intro-scene debugging**: The previous test harness disabled the fade overlay completely with `TP_SKIP_FADE=1`, which made the Nintendo-logo sequence look like it no longer faded in/out. Added `TP_SKIP_FADE_AFTER=<frame>` in `darwFilter()` so headless runs can keep the early fade transitions but still suppress later fade overlays once the gameplay-intro investigation starts. Updated `tools/quick-test.sh` Phase 4 and `.github/workflows/port-test.yml` to use `TP_SKIP_FADE_AFTER=128` and expanded capture frames to include `10,20,40` (plus `129,130`) so CI shows the fade progression and the first visible gameplay-intro frame together. **Local verification:** workflow-style render step now shows progressive logo visibility at frames 10→20→30→40 (avg red ramps `2→5→8→9`) while preserving the gameplay debug frame at frame_0129 (`pct_nonblack=56`, white geometry over black sky). | 17 (stable locally) | **Next session**: (1) improve the gameplay-intro shading so frame_0129 is not just white silhouettes on black, (2) compare the current 56% frame_0129 against the `sub-pr-14` reference (79%) to isolate which TEV/material path is still missing, (3) if CI confirms the new fade captures are useful, update the PR comment framing to call out the logo fade progression frames explicitly. |
