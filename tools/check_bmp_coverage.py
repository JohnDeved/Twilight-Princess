...
Usage:
    python3 tools/check_bmp_coverage.py verify_output/frame_*.bmp

Emits one JSON line per file to stdout:
    {"bmp": "...", "pct_nonblack": N, "nonblack_pixels": M, "total_pixels": T, "avg_rgb": [R, G, B]}

Used by CI and quick-test.sh to verify that rendered frames contain visible content.
A pct_nonblack > 0 confirms the rendering pipeline produced non-black output.
"""
...
def main():
    ...
    for path in sys.argv[1:]:
        ...
        if result.get("pct_nonblack", 0) > 0:
            any_visible = True
            # Check specifically for frame 129 (3D room frame)
            if "frame_0129" in str(p):
                frame_129_visible = True

    if frame_129_visible:
        print('{"result":"visible_3d_frame_confirmed"}')
    elif any_visible:
        print('{"result":"title_screen_only_nonblack"}')
    else:
        print('{"result":"all_frames_black"}')
