...
|---|---|---|---|
| 2026-03-06 | **GXSetCopyClear white-bg fix + tev200 diagnostic extension**: (1) **White background regression fixed** (`gx_render.cpp`): `pal_render_begin_frame()` was using `g_gx_state.clear_color` (set by `GXSetCopyClear`) for `bgfx::setViewClear()`. Before the Nintendo logo, the game calls `GXSetCopyClear(white,...)` for XFB-copy semantics — on PC this bled through to the viewport clear, producing avg_rgb≈[242,242,242] at frame_0010 (white-to-white logo fade regression). Fix: always clear to 0x000000ff (black); the game's clearEfb draw calls handle actual background color. Render-baseline already expects black background + bright logo at ~95% nonblack (min_pct_nonblack=80). (2) **tev200 diagnostic extended** (`gx_tev.cpp`): Added `blend_src`, `blend_dst` (GX blend function source/dest), `bgfx_state` (full computed BGFX STATE bits as hex — depth+blend+cull+write masks), and `view_id` (bgfx view for this draw: 0=normal, 1=centroid, 2=fade) to the frame-200 per-draw dump. This answers "what does bgfx actually receive vs what gx_tev intended" for frame_0200 near-black diagnosis. The `bgfx_state` hex lets you verify that `BGFX_STATE_WRITE_RGB` and correct depth state are set, and that blend equation isn't discarding the gray output. | 17 (stable) | Run CI; verify: (1) tev200 logs in Phase 4 show `const_clr=[200,200,200,255]` AND `bgfx_state` has WRITE_RGB set AND blend_mode=0 (no alpha kill) — if so, output should be visible; (2) frame_0010 avg_rgb drops from ~[242,242,242] to near [0,0,0] (black bg fix verified). If const_clr is dark, RASC fallback not firing — check mat_color/amb_color. If const_clr=gray but frame still dark, check bgfx_state for unexpected blend/depth. J3D texture path: `s0_abcd=[10,15,15,15]` in centroid draws = RASC-only (no texture); TEV stage must reference GX_CC_TEXC for textures to be sampled — J3DTexture::loadGX IS implemented but materials use pure-RASC lighting. |
