...
|---|---|---|---|
| 2026-03-02 | **Stability + cleanup**: (1) Fixed exit cleanup crash — use `_Exit(0)` instead of `exit(0)` for headless test to skip C++ destructors that crash in `dRes_info_c::deleteArchiveRes`. Added NULL guard for stale mNodes/mStringTable in deleteArchiveRes. (2) Fixed WARNING_SCENE profile hardcoded 0x204 → `sizeof(scene_class)`. (3) Audited all 47 active process profiles — all use sizeof(), no mismatches found. (4) Added missing draw lists to PC render path: `dComIfGd_draw2DOpaTop()` (used by title actor) and `dComIfGd_drawOpaList3Dlast()`. (5) NULL model guards for 5 actors: daTitle_c, VRBOX, VRBOX2, WPILLAR, WMARK — prevents crashes when J3D models fail to load. (6) Reduced debug logging noise — removed verbose per-frame prints (frame >= 170), kept stack canary checks. (7) Updated port-progress.md. | 15 (no change) | Title screen 3D model rendering; implement GX lighting stubs; further JKR heap hardening |
