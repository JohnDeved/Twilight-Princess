...
| **Blocking Issue** | Packet chain is structurally valid (`j3d_chain_invalid=0`) and latest CI crash markers now consistently land at `phase=60` (head virtual-dispatch boundary), so the remaining blocker is packet draw execution (not pre-draw list setup). New vtable reference telemetry is now added for `J3DDrawPacket` / `J3DMatPacket` / `J3DShapePacket` so CI can map crashing packet vptrs to concrete subclasses before applying the first subclass-specific PC bypass/fix. |
...
|---|---|---|---|
| **Scene stability** | Map phase=60 crashing packet vptr to subclass (`J3DMatPacket` vs `J3DShapePacket` vs `J3DDrawPacket`), apply first subclass-specific PC bypass/fix, and sustain nonzero play-window dl_draws | 1 session | **P0** |
